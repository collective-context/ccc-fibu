...
/*/////////////// frher in \EURO\INCLUDE\EUR_DOS.h ///////////////////////////
...
/*ษอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป
  ...
  บ                                                                         บ
  ศอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผ*/
/*      Getargs.h       Typedefs und #defines fr getargs (C fr Kenner)
 */
  #define INTEGER         0
  #define BOOLEAN         1
  #define CHARACTER       2
...
/*
 *  MYDIR.H
 *      #defines und typdefs fr die Kommunikation mit der Routine dir().
 *      dir() wird ein Zeiger auf die DIRECTORY-Struktur bergeben.
 *      DIRECTORY-Strukturen werden von mk_dir() erzeugt und durch
 *      del_dir() wieder gel”scht.
 */

  typedef struct
  {
        char     **lastdir;     /* letzte Erweiterung von dirv          */
        int      maxdirs;       /* Anzahl freier Eintrge in dirv       */
        int      nfiles;        /* Anzahl als File benutzter Eintrge   */
        int      ndirs;         /* Anzahl als Directory benutzter Eintr.*/
        long     nbytes;        /* Byte-Zhler fr Files                */
        char     vol_label[12]; /* Volume-Label, falls angefordert      */
        unsigned width : 7;     /* Gr”แe des gr”แten Elementes in dirv  */

                                /* Verschiedene Flags kontrollieren die */
                                /* Arbeitsweise von dir:                */
        unsigned longf      :1; /* Verwende langes Format fr Eintrge  */
        unsigned files      :1; /* Fhre Files in der Liste auf         */
        unsigned dirs       :1; /* Fhre Directories in der Liste auf   */
        ...
        unsigned sort       :1; /* Sortiere hinzugefgte Eintrge ein   */
...
/*--------------------------------------------------------------------------+
 *                                                                          |
 *        MYDOS.H         Verschiedene #defines fr die Arbeit mit DOS      |
 *                                                                          |
 *--------------------------------------------------------------------------+
 *
 *      Typedefs fr den Einsatz von dos().
 *      Beachten Sie, daแ diese Struktur von den verschiedenen Routinen
 *      verwendet werden k”nnen, die Lattice untersttzt (intdos, bdos etc.)
 ...
 *  oder find_next gefllt (DOS-Systemaufrufe 0x4e und 0x4f).
 */
  ...
  typedef struct
    ...
    long    fi_fsize;       /* Bytes 26-27  File-Gr”แe in Bytes */
  ...
  *  auf eine FILE_INFO-Struktur. Beachten Sie, daแ die Makros
  *  C_YEAR und C_SEC MS-DOS-Eigentmlichkeiten beseitigen.
  *
  ...
  *                     wurde. Wird fr ein Backup auf Falsch gesetzt.
