...
/*ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป
  ...
  ศออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผ*/
STATIC	PSSTR pstrDTitel_m = "ndern Formulare";
...
STATIC	CHAR acEin_m[TB_MAX],  acAus_m[TB_MAX],
	     acFoNr_m[TB_MAX], acProt_m[TB_MAX];
...
for(;;)
  ...
  switch(wResult)
	...
	{PSSTR pstr=str("%s_MF.BAT",strExe_g);
	spawnlp(P_OVERLAY, pstr, "_MF", acEin_m,	// P_OVERLAY
	  acAus_m, acFoNr_m, acProt_m,_N);}

	//{PSSTR pstr=str("%sFC",strExe_g);
	//spawnlp(P_OVERLAY, pstr, pstr, acEin_m,
	//  acAus_m, acFoNr_m, acProt_m,_N);}
      break;
    ...
    case DLG_RETURN:
      boBoxEdit_g=JA;
      apTB_m[wBox]->wAttri=aCS_g[wCSInd_g].wCs_dlg;	/* Farbe zurcksetzen  */
  ...
  strcpy(acFoNr_m, acEin_m);
  {PSSTR pstr=strrchr(acFoNr_m, '.');
  if(pstr==NULL) pstr=strrchr(acFoNr_m, '\0');
  if(pstr)
    {*pstr='\0'; pstr--;
    while(pstr>acFoNr_m && isdigit(*(pstr-1)) ) pstr--;
    strcpy(acFoNr_m, pstr);}}
  ...
  {PSSTR pstr=strrchr(acEin_m, '\0');
  if(pstr && isdigit(*(pstr-1)) && isdigit(*(pstr-2)))
    {strcpy(strMan, "D");
    strcat(strMan, pstr-2); strcat(strMan, "\\");}
  else *strMan='\0';}

  if(pstrApp_m) strcpy(strApp, pstrApp_m);
  if(pstrExt_m==NULL) pstrExt_m=strExt_g;

  strcpy(acAus_m, str("%s%s%sFORM.%s", strDat_g,
    strMan, strApp, pstrExt_m));
  strcpy(acProt_m, str("%s\\%sFORM.PRO", pstrUser, strApp));
  } /* end of for */
...
memcpy(&apstrText_m[3][19], acFoNr_m,strlen(acFoNr_m));
