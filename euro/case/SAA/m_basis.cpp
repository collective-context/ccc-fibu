...
บ  M_ParameterUebergabe()   ... auswerten der Paremeterbergabe aus MSDOS.     บ
...
Parameter:
ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ
SWORD	argc		Anzahl der bergebenen Parameter beim Programmstart.
PSSTR	argv[]		Zeiger auf Arrays, der bergebenen Parameter.
PSSTR	pstrApp 	Applikationskennzeichen (z.B. ST fr Stammdatenprogramme)
PSWORD	pwStKenn	bergebenes Steuerkennzeichen zur Programmsteuerung.
...
Beschreibung:
ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ
Beim Programmstart k”nnen Sie verschiedenste Parameter bergeben. Diese Kenn-
zeichen werden mit dieser Funktion fr Ihre Applikation ausgewertert und an
Ihr Programm bergeben.

Es gibt drei Varianten des Programmstarts:

๙ <ProgrammName>

๙ <ProgrammName> <Steuerkennzeichen>

๙ <ProgrammName> <Steuerkennzeichen> <Applikation>
                 <MaskenNummer> [ <MaskenNummer> ...]

argv[0] = Programmname  (z.B. ST1100.EXE)  ฏDOS-Konfention beachtenฎ
argv[1] = Steuerkennz.  (z.B. 02000) ฏ2 Zeichen = 1xINTฎ ฏ3 Zeichen = 1xSTRINGฎ
argv[2] = Applikation   (z.B. ST = Stammdaten) ฏ2 Zeichen = 1xSTRINGฎ
argv[3] = Maskennummer  (z.B. 110) ฏ3 Zeichen = 1xINTฎ
argv[+] = ... weitere Maskennummern

Ihre Applikation sollte so programmiert sein, daแ vor der Funktion folgende
...
wSteuerKennzeichen_m = 0;          wenn keine Parameterbergabe ฏ Kundenstamm
...
siert verwendet, die Funktion berschreibt keine Werte.

Bei der zweiten Variante des Programmstarts wird aus MSDOS ein ein- bis fnf-
stelliger Wert bergeben. Aus den ersten beiden Stellen wird ein INT-Wert
gebildet und an wSteuerKennzeichen_m zurckgegeben. Die nchsten drei Zeichen
werden als String an strSteuerkennzeichen_m bergeben.

Bei der dritten Variante wird zustzlich ein String (empfohlen zwei Zeichen)
an strApp_m fr die Applikation bergeben. Weiters werden die Maskennummern
als INT-Wert an awMask_m bergeben. Die Funktion bildet die Dateinamen mit den
...
den aus MSDOS bergebenen Dateinummern (empfohlen vier Zeichen) und als
Extention .BTR!
...
i_InitVars();
if(argc > 1)					 /* Falls Argumente-bergabe */
  {
...
if(argc > 2 && *argv[2]=='/')			 /* Falls Argumente-bergabe */
  {
...
/* pWkbTest=Wi_Einrichten(3, 11, 72, 11);	     /* Fenster fr	     */
...
if(strlen(pSkb_g->strManName)==0)		       /*		       */
  sprintf(pSkb_g->strManName, "ฎ%sฏ", strMan_g);       /*		       */

sprintf(strMandant, "%s %s "                         /* Mandant zentrieren u.*/
  "ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ",            /* mit 'ฤ' einschlieแen */
  strstring(strTemp, 'ฤ',                            /*                      */
...
Vi_Swza(0,0,80,2,' ',aCS_g[wCSInd_g].wCs_mz_c);      /* Kopfzeile  l”schen   */
Vi_Swza(0,24,80,1,' ',aCS_g[wCSInd_g].wCs_mz);       /* Hilfszeile l”schen   */

sprintf(strText,				   //
  "ฺ%s ฤฤฤฤ%sฤฤฤ Datum %02d.%02d.%s (%s)ฟ",	       //
...
Vi_Sz(0, 1, (UCHAR)'ภ');			     /* Kopfzeile 2 ausgeben */
Vi_Sz(79,1, (UCHAR)'ู');			     /* Kopfzeile 2 ausgeben */
...
wRetCode=BTRV (B_CLOSE, strFileBlock, strNull,	     //
  ...
  sprintf(strTemp,"M_BasisMaske(B_CLOSE), - "	      //
    "Datei zum Schlssel: %s", strMan); 	  // ~
 // "Datei zum Schlssel: %s", strMan, 0);	     // ~
  Dl_ErrorHandler (wRetCode, strTemp,		      //
    __FILE__, __LINE__, 0);			     //

ncpy(pSkb_g->strCopyright, &pstrDataBuffer_g[62], 16);	// [46], 17 frher
...
strcpy(pSkb_g->strManName,   &pstrDataBuffer_g[  21]);	  // [ 13] frher
strcpy(pSkb_g->strDatumAkt,  &pstrDataBuffer_g[ 351]);
strcpy(pSkb_g->strNutzung,   &pstrDataBuffer_g[1479]);	  // [ 69] frher
