&TITEL     = "FI  PROG Buchungs-Erfassung"

&FORMULAR    = 10
&FOOT_H      = 2

&DATEI (0) = MASK, FI2100
&DATEI (1) = MASK, FI4210
&DATEI (2) = MASK, SYP_01

&KOPF (0)
^зыBUCHUNGSERFASSUNGS-PROTOKOLL                                                          Datum: @@@@@@@@ - SEITE ⌡⌡⌡⌡ю© ~SM ~1._DATUM_AKT ~1._SEITE
Ёздддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©Ё
ю╢Mandant (⌡⌡) @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@                                                        Teilnehmer: ⌡⌡⌡цы ~1._NR_MAND ~1._MANDANT ~1._TEILN
 юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
&END

&RUMPF (0)

дддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд
з:SATZ_NR :BELEG_NR :BELEG_DAT :BUCH_DAT :SOL_KONTO :HAB_KONTO    :BETRAG_BR :UST_ART :UST_SATZ       :UST_BETRдддддд©
юдд⌡⌡⌡⌡⌡⌡ @@@@@@@@@@  ##.##.##  ##.##.## @@@@@@@@@  @@@@@@@@@ ###.###.##⌡,⌡⌡        @    @@@@@@   ##.###.##⌡,⌡⌡дддддды ~0.:SATZ_NR~0.:BELEG_NR~0.:BELEG_DAT~0.:BUCH_DAT~0.:SOL_KONTO~0.:HAB_KONTO~0.:BETRAG_BR~0.:UST_ART~0.:UST_SATZ~0.:UST_BETR
з:OP_NUMMER :BUCH_ART  :ZAHL_KO  :SKO_BET1 :SKO_BET2 :SKO_BET3 :SKO_BET4 :SKO_BET5 :SKO_BET6 :SKO_BET7 :SKO_BET8ддддд©
ю@@@@@@@@@@        ⌡⌡  @@-@@-@@@ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ ##.##⌡,⌡⌡ддддды ~0.:OP_NUMMER~0.:BUCH_ART~0.:ZAHL_KO~0.:SKO_BET1~0.:SKO_BET2~0.:SKO_BET3~0.:SKO_BET4~0.:SKO_BET5~0.:SKO_BET6~0.:SKO_BET7~0.:SKO_BET8
з:K_AUFTEIL :BUCH_TEXTдддддддддддддддддддддддддддддддддд©
юддддддддд@ @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@дддды        ~0.:K_AUFTEIL~0.:BUCH_TEXT
&END

&REM ##.##.## @@@@@@@@@ @@@@@@@@@@ ##.##.## @@@@@@@@@ ###.###.###.## @@@@@@@@@@@@@@@@@@@@@@@@ @ @@@@@@ ##.###.###.## ~0.SATZ_NR ~0.BUCH_DATUM ~0.SOLL_KONTO ~0.BELEG_NR ~0.BELEG_DAT ~0.HAB_KONTO ~0.BETRAG_BR~0.BUCH_TEXT~0.UST_ART~0.UST_SATZ~0.UST_BETRAG

&FUSS (0)
здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
ю©Nutzungsberechtigung: @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@      @@@@@@@@@@@@@@@@@зы ~1._NUTZUNG  ~1._COPYRIGHT
&END


