 ...
 *                    VerfÅgung, die Datum und Uhrzeit lesen     *
 *                    und setzen.                                *
 *   Memory Model:    SMALL                                      *
...
if (d == NULL    /* Falls Aufrufer NULL Åbergibt fordert die
                             Funktion den nîtigen Speicher an */
    && (d = (struct D_DATE *)
...
return (d);           /* Gibt die Adresse der Struktur zurÅck */
}
...
return (inregs.h.al);  /* 00 = gÅltiges Datum, FFh = ungÅltig */
}
...
inregs.h.ah = SET_TIME;            /* FÅllen der Register mit */
inregs.h.ch = t->hour;                /* der gewÅnschten Zeit */
inregs.h.cl = t->min;
inregs.h.dh = t->sec;
inregs.h.dl = t->hsec;
