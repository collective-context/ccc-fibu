// (C)WINware Software, P.Mayer  Letztes Update am 14-Feb-1996 / 11:18:29 - Wed
...
if(wFehler)
  ...
  Ut_SchreibArray(apstrMessage,
    "  Fehler im Schlssel: ฺ NR-JJ.MM.TT ฟ  ",
    "                       ภ             ู  ",    /*                      */
...
memcpy(&apstrRecord_g[0][awRecKeyOffset_g[0]],       /*                      */
    apstrRecKey_g[0], awRecKeyLength_g[0]);          /* Schlssel eintragen  */
...
//ษออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออป
//บ Au230_Prepare_Save_Record() 					     บ
//บ ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ บ
//บ In dieser Funktion werden die Daten aus Lagerbewegungen erfassen in      บ
//บ die Lagerstnde bertragen. 					     บ
//ศออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผ
/*GLOBAL
...
UWORD uwLager, uwLagerVor, uwTmpLager;
...
if(*pt("ART_NR"))
{
  while(pTB->wOffset+wBlock < (SWORD)_msize(apstrRecord_g[pTB->wRecord]) )
    ...
    memcpy(&uwLager, pt("LAG_ORT")+wBlock, 2);
    memcpy(&dMengeVor, pt("MENGE_VOR")+wBlock, 8);
    memcpy(&uwLagerVor, pt("LAG_VOR")+wBlock, 2);
...
boTestModus_g=EIN;
  Wi_TestPrintf(pWkbInfo_g, "\n\ndBlock (%d)", wBlock);
  Wi_TestPrintf(pWkbInfo_g, "\ndMenge (%.2f)", dMenge);
  Wi_TestPrintf(pWkbInfo_g, "\ndMenge_Vor (%.2f)", dMengeVor);
  Wi_TestPrintf(pWkbInfo_g, "\ndLager (%d)", uwLager);
  Wi_TestPrintf(pWkbInfo_g, "\ndLager_Vor (%d)", uwLagerVor);
  Wi_TestPrintf(pWkbInfo_g, "\ndDifferenz (%.2f)", dTemp);
boTestModus_g=AUS;
...
//  memcpy(pt("LAG_VOR")+wBlock, &uwLager, 2);


boTestModus_g=EIN;
  Wi_TestPrintf(pWkbInfo_g, "\nA: dMenge (%.2f)", dMenge);
    memcpy(&dMenge, pt("MENGE_VOR")+wBlock, 8);
  Wi_TestPrintf(pWkbInfo_g, "\nB: dMenge aus Menge_VOR (%.2f)", dMenge);
boTestModus_g=AUS;
    ...
    for(wCounter=1;wCounter < 3; wCounter++)
      {
      boAendern=NEIN;
      if(uwLager != uwLagerVor && uwLagerVor != 0)
	{if(wCounter == 1)
	   {uwTmpLager = uwLagerVor;
	   dTemp=dMengeVor;}
	if(wCounter == 2)
	   {dTemp=dMenge;
	   uwTmpLager = uwLager;}
	boAendern=JA;
      ...
      if(dTemp && (uwLager == uwLagerVor || uwLagerVor == 0))
	{wCounter=3; uwTmpLager=uwLager; boAendern=JA;}

      if(boAendern)
	{Read_Rec(wFH,pt("ART_NR")+wBlock,0,JA,B_MNW,_F,_L);

	memcpy(&dMengeLag, pt("LAG_00-99")+(uwTmpLager * 8), 8);
	if(wCounter != 1)  dMengeLag+=dTemp;
	else dMengeLag-=dTemp;
	memcpy(pt("LAG_00-99")+(uwTmpLager * 8), &dMengeLag, 8);

	memcpy(&dLagGes,pt("LAGER_GES"), 8);
	if(wCounter != 1)  dLagGes+=dTemp;
	else dLagGes-=dTemp;
	memcpy(pt("LAGER_GES")+(uwTmpLager * 8), &dLagGes, 8);

// E();
boTestModus_g=EIN;
  Wi_TestPrintf(pWkbInfo_g, "\ndMenge Lager (%.2f)", dMengeLag);
  Wi_TestPrintf(pWkbInfo_g, "\ndMenge Lager Gesamt (%.2f)", dLagGes);
boTestModus_g=AUS;
...
//บ Au230_Prepare_Save_Record() 					     บ
//บ ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ บ
//บ Nur fr das auf Null Setzten der Lagermengen			     บ
//บ									     บ
//ศออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผ
GLOBAL
VOID Au230_Prepare_Save_Record()
...
SWORD wBlock=0;
SWORD wFH=pTBArt->wRecord;
DOUBLE dNull=0.0, dLager, dLagGes, dTemp, dTemp1;
...
if(*pt("ART_NR"))
{
  while(pTB->wOffset+wBlock < (SWORD)_msize(apstrRecord_g[pTB->wRecord]) )
    ...
    memcpy(pt("LAG_VOR")+wBlock, &dNull, 8);
  ...
  for(UWORD i=0; i<100 ; i++)			      // Diese Schleife
    ...
    wBlock+=awBlockLength_g[_MSK];		      // nicht ben”tigt.
   ...
   memcpy(&dTemp1, pt("LAG_VOR")+wBlock, 8);

boTestModus_g=EIN;
  Wi_TestPrintf(pWkbInfo_g, "\nMenge vor (%.2f)", dTemp);
  Wi_TestPrintf(pWkbInfo_g, "\nLager vor (%.2f)", dTemp1);
  Wi_TestPrintf(pWkbInfo_g, "\nLager gesamt nach nderung(%.2f)", dLagGes);
  Wi_TestPrintf(pWkbInfo_g, "\nMenge Lagerort nach nderung(%.2f)", dLagGes);
boTestModus_g=AUS;
