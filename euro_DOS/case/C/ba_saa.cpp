  ...
  case T_F5:                                         /* Kopie des Datensatzes*/
    ...
    if(awExistingRecord_g[0] && CopyDoesNotExist())  /* wenn Record da, dann */
      ...
      if(wKeyCode_g==T_S_F5)
        ...
        if(awExistingRecord_g[0])                    /* Falls vorhanden      */
          {Delete_Record(); wEscFlag_g=NO;           /* Satz l”schen und n. */
...
for(i=0; i<wMaxRecKey_g; i++)                        /* fr alle Schlssel   */
  if(awRecKeyField_g[i]==_FLD &&                     /* prfen, ob man in    */
    ...
    {                                                /* Wenn JA, dann berge-*/
    wRecKey_g=i+1;                                   /* ben ob 1,2... Schls */
    i_Field_Classification(&wRefKey_g, &wRefFile_g); /* fr Browse in FI1100 */
    return(ABBRUCH);                                 /*                      */
    }
...
if(pMATCH_FLD==NULL)
  ...
  {SWORD wFH=99;
  if(isdigit(*pMATCH_FLD) && *(pMATCH_FLD+1)=='๘')
...
Field_Classification();
if(wKey != 0 || !memcmp(strOldRecord_m,              /* Kein Primrkey-Feld  */
