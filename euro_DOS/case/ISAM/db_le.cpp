...
º  Db_GetLe()	liest einen Datensatz gleich oder kleiner dem Schlssel        º
...
#include <eur_btr.h>
SWORD Db_GetLe(wFileHandler, ppstrFileBlock, ppstrRecord, pwRecLength, wLock,
	     pstrRecKey, wKeyNumber, pstrErrMark);

Parameter fr GreaterOrEqual:
ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
...
/*ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»
  ...
  ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼*/
GLOBAL
SWORD Db_GetLe(SWORD wFile,	 PPSTR ppstrBlock,   PPSTR ppstrRecord,
...
while(wSelect!=0)
  ...
  wStatus=BTRV(B_GET_LE+wLock, *(ppstrBlock+wFile),  /* 		     */
  ...
  sprintf(strError,"Db_GetLe(%d), - File/Key: "      /* 		     */
...
while(wLock!=0)
  ...
  switch(wStatus)
    ...
    case 84:					     // Record in Use
      wHandle=1;
      M_OkQuestion(&wLock, "Datensatz " 	     // Antwort
	"gesperrt - Zugriff wiederholen ?",	     // Titelzeile
	"#Nein, Zugriff nicht wiederholen.",	     // Text der Auswahl-
	"#Ja, Zugriff wiederholen.",_N);	     // liste
      break;
    ...
    case 85:					     // Record in Use
      wHandle=1;				     //
      M_OkQuestion(&wLock, "Datei "		     // Antwort
	"gesperrt - Zugriff wiederholen ?",	     // Titelzeile
	"#Nein, Zugriff nicht wiederholen.",	     // Text der Auswahl-
	"#Ja, Zugriff wiederholen.",_N);	     // liste
      break;
  ...
  sprintf(strError,"Db_GetLe(%d), - File/Key: "        //
    "%d/%d _ %s-%d.%d.%d.%d.%d", wLock, wFile, wKeyNr, //
    pstrRecKey,*pstrRecKey,*(pstrRecKey+1),	       //
    *(pstrRecKey+2),*(pstrRecKey+3),*(pstrRecKey+4) ); //

  Dl_ErrorHandler(wStatus,strError,pF,wL,wHandle);     // Status 4 (key not
  } // end while(wLock) 			       // found) ¯ wHandle==1
						       // found) ¯ wHandle==1
wKeyNumber_g=wKeyNr;
return(wStatus);
} // end Db_GetLe ()
