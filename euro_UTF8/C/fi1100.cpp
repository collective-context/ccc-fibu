...
if(Maske(_MSK,"FI145","LKF145",_N))
  ...
  for(j=0; j<4; j++)
    ...
    for(i=1; i<13; i++)
      {adSaldo[i]=ptD(chri(chi(j,"x°SOLL_0x"),i))-
	ptD(chri(chi(j,"x°HABEN_0x"),i));

      dJahrSoll+=ptD(chri(chi(j,"x°SOLL_0x"),i));
      dJahrHaben+=ptD(chri(chi(j,"x°HABEN_0x"),i));
      pstr=ch((CHAR)(j+48),"SALDO_x_0x",6);
      memcpy(pt(chri(pstr,i)),&adSaldo[i],8);
      }

    dJahrSaldo=ptD(chi(j,"x°VOR_SALDO"))+dJahrSoll-dJahrHaben;
...
if(Maske(_MSK,"FI160","LKF160","FI165","LKF165",_N))
  ...
  WJ_ZUGANG   >>Zugang
  AO_ABGANG   >>außerordentlicher Abgang
  WJ_AFA      >>Abschreibung
...
{
SWORD  wEmpty=NEIN;				       /* Flag für ungültige   */

wFail_g=(
  (wKeyCode_g==T_C_PGDN || wKeyCode_g==T_C_PGUP) &&
  (!awNewRecord_g[0] && !awExistingRecord_g[0]) );

wEmpty=wEmpty||(strKeyValue_g[0]=='\0');	     /* Flag für ungültige   */
if(wEmpty)
  MissingKey();
return(wEmpty);
...
if(Maske(_MSK,"FI310","LKF310","FI320","LKF320",
  ...
  {
  DOUBLE dBetrag=ptD("_VOR_SALDO")-ptD("0°VOR_SALDO");
  DOUBLE dNull=0.0, dSaldo;

  i_UpdateBestKto(dBetrag);

  dSaldo=ptD("0°VOR_SALDO")+dBetrag;
  memcpy(pt("0°VOR_SALDO"), &dSaldo, 8);
