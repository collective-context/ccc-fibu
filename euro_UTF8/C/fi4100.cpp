...
// ║  Function	    : Drucken euroSOFT-Stammdaten, ...			      ║
// ║									      ║
// ║									      ║
...
/*sCho("K_STELL_KZ","_W_KOST_KZ");
  ...
  sCho("KONTENART","_W_KTO_ART");}  früher */
...
/* Wi_TestPrintf(pWkbInfo_g, "\n»%s«»%s«.", pt(acVonWahl_g), pt(acBisWahl_g)); */
...
if(Maske(_MSK,"FI4140","LKF4140",_N) &&
  ...
  Fkb_g.wNextHeader=Fkb_m.wNextHeader;               /* nächster Kopf        */
  Fkb_g.wNextFoot=Fkb_m.wNextFoot;                   /* nächster Fuß         */
  }
...
/*if(Maske(_MSK,"ST4113",_N))
  wEtikettHoehe_g=atoi(pt("_HÖHE")) -9;*/

/*else if(Maske(_MSK,"ST4123",_N))
  wEtikettHoehe_g=atoi(pt("_HÖHE")) -8;*/

/*else if(Maske(_MSK,"ST4142",_N))
  wEtikettHoehe_g=atoi(pt("_HÖHE")) -4;*/
...
if(Maske(_MSK,"FI4315",_N) &&
  ...
  memcpy(pt("_SUMME_HA"),  &dSummeHab_g, 8);}          // Zeile der nächsten Seite mitge-
                                                        // rechnet wird
...
/* if(Maske(_MSK,"XXnnnn",_N))
  ...
  memcpy(strAktMenue, pt("MENÜGRUPPE"), 7);

  if(!memcmp(strVorMenue,strAktMenue, 7))
    strcpy(pt("MENÜGRUPPE"), "\0");
...
---
Wäre eine bessere Möglichkeit; haben aber Stack-Größenprobleme:
...
if(Maske(_MSK,"FI4134","LKF4134","FI4140",           // Erfolgskonten
  ...
  while(wSelect!=2)
    {M_OkQuestion(&wSelect, "Ausdruck auf %s",
      pt("DR_NAME"), "Ausdruck der #Daten.", "#Neuen Drucker wählen.",
      "#Abbruch ohne drucken.", "Zurück zur #Eingabe.", _N);

    switch(wSelect)					 // Bei ESC oder Zurück
      {case -1: 					 // zur Eingabe die Ver-
      case   3: return(NEIN); break;			 // arbeitung verzögern
