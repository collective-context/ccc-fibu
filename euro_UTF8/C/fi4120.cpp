// (C) FiCore - Open Source Financial Core  Letztes Update am 14-Feb-1996 / 12:08:59 - Wed

//////////////////// Wird nicht mehr verwendet /////////////////////////////
/*╔═════════════════════════════════════════════════════════════════════════╗
  ║  Programme-Name: FI4100.C        Revision: 1.2                          ║
  ║  Function      : FiCore FIBU-Stamm - Listen Drucke                    ║
  ║                                                                         ║
  ║                                                                         ║
...
MGLOBAL DOUBLE dAnfEbWert_m=0.0, dEndEbWert_m=0.0;           /* Kontoblätter */
...
switch (wSteuerKennzeichen_m)
  ...
  case 10:  /*110: in FI4100.C			     /* FI-Kundenstamm	     */
    ...
    switch (wDruckKennzeichen_m)
        {
        case 10:
          strcpy (strAppDru_m, "FI");                /* Formular für Auf-    */
  ...
  case 11:  /*111: in FI4100.C			     /* ST-Stamm	     */
    ...
    switch (wDruckKennzeichen_m)
        {
        case 0:
          strcpy (strAppDru_m, "FI");                /* Formular für List    */
  ...
  case 15:  /*115: in FI4100.C			     /* 		     */
    ...
    switch (wDruckKennzeichen_m)
        {
        case 0:
          strcpy (strAppDru_m, "ST");                /* Formular für Mahnung */
          wFormular_m = 2100;                        /*                      */
          break;
        }
    break;
  ...
  case 30:  /*130: in FI4100.C			     /* FI-Stamm Anlegen     */
    ...
    switch (wDruckKennzeichen_m)
        {
        case 0:
          strcpy (strAppDru_m, "FI");                /* Formular für Liste   */
  ...
  case 40:  /*210: in FI4100.C schon übernommen      /* FI-Druck	     */
  ...
  case 50:  /*310: in FI4100.C schon übernommen      /* FI-Druck	     */
  ...
  case 60:  /*330: in FI4100.C schon übernommen      /* FI-Druck	     */
    strcpy (strApp_m, "FI");                         /* Kontoblätter         */
    awMasks_m[0] = 4230;                             /*                      */
    awMasks_m[1] = 36;                               /* Dummy Maske zum Druck*/
    awMasks_m[2] = NULL;
    switch (wDruckKennzeichen_m)
...
while (!wFinis_m)                                    /* Haupt-Eingabe-Schl. */
  ...
  while( wKeyCode_m != 9999 && !wEscFlag_m )         /* ESC-Taste gedrückt   */
    ...
    Key_Code_Evaluation(apWkbMask_m, wKeyCode_m,    /* Für Application_Set  */
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ...
  ║ liche Feldinhalte. Bei erfolgreicher Prüfung muß wValid auf JA  (1)     ║
  ║ gesetzt werden. Fällt die Prüfung durch muß wVailid auf NEIN (0) ge-    ║
  ║ setzt werden.                                                           ║
  ╚═════════════════════════════════════════════════════════════════════════╝*/
MGLOBAL
SWORD Semantic_Check ()
{
wValid_m=JA;                                         /* Datensatz gültig     */
...
awFileMode_m[pTB->wRecord]=REF_F;                    /*  Ref_Key n. löschen  */
...
memcpy(apstrRecKey_m[wNdx_m],                        /* Primärdruckrekord    */
  apTextBox_m[0][0]->acText, awRecKeyLength_m[wNdx_m]);

if(Mask("FI4210")||Mask("FI4312"))                   /* Buchungserfassungs-Pr*/
  memcpy(apstrRecKey_m[wNdx_m], pt("_V_SATZNR"), 4); /* im Datensatz»binär   */
...
if(!Mask("XXnnnn"))                                /* alle außer nnnn        */
  {
   SWORD wBisSeite;
...
if ( !(pfDevice_m = fopen(strDevice, "w")) )    /* Protokoll - Datei öffnen */
   {
...
if ( fclose(pfDevice_m) )                       /* Eingabe - Datei schließen */
   {
...
while(wEnd < 0)                                     /* wenn rec < key bis   */
  ...
  if(Mask("FI4110") )                                /* Kunden-Liste         */
    {
    pstrKey=pt("KU_KONTO");                          /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KONTONR");                     /* bei Key 2            */
    }

  if(Mask("ST4210"))                                 /* Lieferanten-Liste    */
    {
    pstrKey=pt("LI_KONTO");                          /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KONTONR");                     /* bei Key 2            */
    }

  if(Mask("FI4111") || Mask("FI4121"))               /* Kunden-Liste         */
    {                                                /* Lieferanten-Liste    */
    pstrKey=pt("KURZ_BEZ");                          /* Primärdurckrekord    */
  ...
  if(Mask("FI4130"))                                 /* Sachkontenstamm      */
    {
    pstrKey=pt("KONTONR");                           /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KTONR");                       /* bei Key 0            */
    }

  if(Mask("FI4140"))                                 /* Bankenstamm          */
    {
    pstrKey=pt("BANKLEITZ");                         /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KTONR");                       /* bei Key 0            */
    }

  if(Mask("FI4150"))                                 /* Bilanztexte          */
    {
    pstrKey=pt("BIL_KZ");                            /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KTONR");                       /* bei Key 0            */
    }

  if(Mask("FI4170"))                                 /* Kostenstellen        */
    {
    pstrKey=pt("KOSTENSTEL");                        /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KTONR");                       /* bei Key 0            */
    }

  if(Mask("FI4171"))                                 /* K-Stellen/K-Arten    */
    {
    pstrKey=pt("K_STEL_ART");                        /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KTONR");                       /* bei Key 0            */
    }

  if(Mask("FI4112") || Mask("FI4122"))               /* Kunden-Liste         */
    {                                                /* Lieferanten-Liste    */
    pstrKey=pt("MATCH_CODE");                        /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KURZBEZ");                     /* bei Key 1            */
    }

   if(Mask("FI4210"))                                /* Buchungserfassungs-Pr*/
    {
    memcpy(&lKey,pt(":SATZ_NR"),4);                  /* Primärdurckrekord    */
  ...
  if(Mask("FI4230"))                                 /* Kontoauszug          */
    {
    pstrKey=pt(".KONTO_NR");                         /* Primärdurckrekord    */
    pstrKeyEnd=pt("_B_KTONR");                       /* bei Key 0            */
...
dAnfEbWert_m=dEndEbWert_m=dAnfJvkzSo_m=dEndJvkzSo_m=0.0;  /* Kontoblätter */
...
if(Mask("FI4110") || Mask("FI4111"))                 /* Kundenkonten         */
  ...
  strcpy(pt("_WÄHRUNG"), "\0");
  for(i=0; i<10; i++)
    if(*pt("WÄHRUNG") == (CHAR)i+48 )
      strcpy(pt("_WÄHRUNG"), astrFremd_m[i]);
  }
...
for(i=0; i<wFileHandler_m; i++)                      /* Dateien öffnen       */
  if(awFileMode_m[i] != DRU_F)                       /* Außer Druckdateien   */
