...
if(argc > 2 && argc < 4)			    /* Argument übergeben ? */
  {
...
SWORD	 wRet;
    ...
    if(!(pFileHandleEin=fopen(pstrDateiEin_m,"rb")) ) /* Eingabe-Datei öffnen */
      {
      printf ("\nEingabe-Datei <%s> kann nicht geöffnet werden", pstrDateiEin_m);
      exit (ERROR);
      }

    if(!(pFileHandleAus=fopen(pstrDateiAus_m,"wb")) ) /* Ausgabe - Datei öffnen */
      {
      printf ("\nAusgabe-Datei <%s> kann nicht geöffnet werden", pstrDateiAus_m);
      exit (ERROR);
      }
    ...
    if( fclose(pFileHandleEin) )		    /* Eingabe - Datei schließen */
      {
      printf ("\n\nEingabe-Datei <%s> kann nicht geschlossen werden", pstrDateiEin_m);
      exit (ERROR);
      }

    if( fclose(pFileHandleAus) )		    /* Ausgabe - Datei schließen */
      {
