// https://Creative-Context.org  ->        Letztes Update: 2025-11-13 / 22:08
/*╔═════════════════════════════════════════════════════════════════════════╗
...
M_BasisMaske("[ESC]=Beenden  [F1]=Hilfe  "
   "[F2]=Suchen  [F3]=Löschen  [F10]=Menü  ");
Mn_Aktivieren(1);                                    /* Menüzeile anzeigen   */

if(wSteuerKz_g==99)                                  /* Resverviert für Test */
...
/*{BOOL boTest=boTestModus_g; boTestModus_g=JA;
  ...
  if(lBlockVer_g&lBlock &&
    ...
    awBlockLength_g[i]= 			     /* Im Block höchstes    */
    apTextBox_g[i][awBlocks_g[i][1]]->wOffset -      /* Offset - niedrigstes */
    apTextBox_g[i][awBlocks_g[i][0]]->wOffset +      /* Offset + max. Länge  */
...
for(i=0; i<wFileHandler_g; i++)
  {awExistingRecord_g[i]=NO;			     /* Flag rücksetzen      */
  awNewRecord_g[i]=NO;				     /* Flag rücksetzen      */
  awChangeFlag_g[i]=NO;
...
if(*(pstrTitel+1)==(CHAR)'─') {wBreite=atoi(pstrTitel); pstrTitel+=2;}
if(*(pstrTitel+2)==(CHAR)'─') {wBreite=atoi(pstrTitel); pstrTitel+=3;}
...
if(*pt("Y_CLOSED")=='j')
    ...
    Ut_SchreibArray(apstrMessage,
      ...
      " ",
      "Es können nur mehr Auswertungsprogramme ",
      "gestartet werden.",_N);
    Dl_Info(apstrMessage, DLG_INFO);
