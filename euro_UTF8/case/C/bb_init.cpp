...
// บ  Programme-Name: BB_INIT.C       Revision: 1.3	    (C) 1989/1991     บ
...
for (pTBI=pInitMasks_g->pBlInitWurzel->pTBLWurzel;
        pTBI && strcmp(pTBI->strDatenfeld, "WHRUNG");
     pTBI=pTBI->pNext)
   ;

if(!pTBI)
  {Dl_Abbruch("ReadChoiceFremd", _L, _F, "WHRUNG");
  goto READMWST;}

Ut_Calloc(pTBI->pstrChoiceTitel, 80, CHAR);
strcpy(pTBI->pstrChoiceTitel,"Fremdwhrung");

Ut_Calloc(pTBI->pChoice, 1, CHOICES);
pLauf=pTBI->pChoice;

pstr=Bpoint("0.(null)","FRW_BE_00", 1, JA, _F, _L);
if(!*pstr)
  {strcpy(strTemp, " ");
  pstr=strTemp;}
...
for(i=1; i<10; i++)
  {
//pstr=p0(ch((CHAR)(i+48),"FRW_BE_0x",9));
  pstr=Bpoint("0.(null)",ch((CHAR)(i+48),"FRW_BE_0x",9), 1, JA, _F, _L);
  if(*pstr)
    {
...
for (pTBL=pInitMasks_g->pNext->pBlLineWurzel->pBlWurzel->
        pTBLWurzel; pTBL &&
        strcmp(pTBL->pstrDatenfeld, "WHRUNG");
        pTBL=pTBL->pNext)
   ;
