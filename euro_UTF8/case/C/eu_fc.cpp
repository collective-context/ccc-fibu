...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ║ Source_Open ()                                                          ║
  ║ ─────────────────────────────────────────────────────────────────────── ║
  ║ Öffnen der Eingabedatei, einlesen des Source, schließen der Eingabe-    ║
  ║ datei, öffnen der Protokolldatei ...                                    ║
  ╚═════════════════════════════════════════════════════════════════════════╝*/
GLOBAL
...
*ppFileHandle=fopen(argv[1], "r");		     /* Source-File öffnen   */
if(!(*ppFileHandle))
  {                                                  /*                      */
  printf ("\nEingabe-Datei <%s> kann nicht "         /*                      */
    "geöffnet werden", argv[1]);                     /*                      */
  return(ERROR);
  }
...
for(i=0; fgets(pstrDataBuffer_g,MAX_RECORD_SIZE -1,
  *ppFileHandle); )    /* Lies und verweile    */
  ...
  if(boSameLine==NEIN)
    ...
    strSource[ strlen(strSource)-1 ] = '\0';         /* LF rauslöschen       */
...
if(*ppstrZeile == NULL)                              /* Gültigkeit prüfen    */
  return(ERROR);


if( fclose(*ppFileHandle) )                          /* Source-File schließen*/
  {                                                  /*                      */
  printf ("\nEingabe-Datei <%s> kann nicht "         /*                      */
...
*ppFileHandle=fopen(argv[4], "w");		     /* Protokolldatei öffn. */
if( !(*ppFileHandle) )
  {                                                  /*                      */
  printf ("\nProtokoll-Datei <%s> kann nicht "       /*                      */
    "geöffnet werden", argv[4]);                     /*                      */
  return(ERROR);
  }
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ║ Destination_Open ()                                                     ║
  ║ ─────────────────────────────────────────────────────────────────────── ║
  ║ Öffnen oder Erstellen der Formular-Bibliotheks-Datei.                   ║
  ╚═════════════════════════════════════════════════════════════════════════╝*/
GLOBAL
...
fsMaskBuffer.wNdxCnt = 1;                            /* Index-Einträge       */
fsMaskBuffer.ksKeyBuf[0].wKeyPosition = 7;           /* Position 1 Stelle    */
fsMaskBuffer.ksKeyBuf[0].wKeyLength = 10;            /* Länge 10 Zeichen     */
...
wRetCode=BTRV(B_OPEN,pstrFileBlock,pstrDataBuffer_g, /* Datei öffnen         */
...
Dl_ErrorHandler(wRetCode, strError,                  /*                      */
  ...
  if(wRetCode == 12)
    ...
    wBufferLen=0;
    wRetCode=BTRV (B_OPEN, pstrFileBlock,            /* Datei öffnen         */
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ║ Destination_Close ()                                                    ║
  ║ ─────────────────────────────────────────────────────────────────────── ║
  ║ Schließen der Formular-Bibliotheks-Datei.                               ║
  ╚═════════════════════════════════════════════════════════════════════════╝*/
GLOBAL
