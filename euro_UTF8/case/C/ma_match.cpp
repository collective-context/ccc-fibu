...
GLOBAL	BOOL boMaskAkt_g;			     /* Fr Match_Check_OK() */
...
PTEXTBOX pT=*pMatchCode->ppKey;      /* flexiblere Anzeige, wenn erstes Feld */
				     /* ungleich ausgewhltes Feld	     */

boMaskAkt_g=NEIN;				     /* Fr Match_Check_OK() */
apfsFile_m = apfsFile;
...
if (wZeile  < 3)
  wZeile=3;				       /* Wert fr Dialogfenster    */
...
SWORD i_EventBearbeiten(PPSTR ppstrFileBlock)
{
  ...
  for(;;)                                           /* Eingaberoutine       */
  {
	...
	if (pEvent_m->wTaste == T_TAB ||
	  ...
	  if(wFokus_m==VERZFELD) i_AendereFokus();   /* evtl. Fokus ndern   */
	  i_Dl_LiesZK (pTextBox_m, pEvent_m, AKT_BUFFER);}
...
/* #endif */
	...
	if (pEvent_m->wZeile == wZeile_m+HOE-2) 	   /* Schaltflchen-Zeile  */
        {

	  if (pEvent_m->wSpalte > wSpalte_m+2 &&	    /* RETURN-Schaltflche  */
	  ...
	  else if (pEvent_m->wSpalte > wSpalte_m+16 &&	    /* ESCAPE-Schaltflche  */
	  ...
	  else if (pEvent_m->wSpalte > wSpalte_m+30 &&	    /* HILFE-Schaltflche   */
...
wAktZeile_m = 0;
  ...
  i_RecordToBox(pT, 1); 		   /* Notwendigkeit hier prfen !!!!! */
  i_KopierBox(pTextBox_m, pT);
...
if(!*ppstrEnde_m)
  ...
  Ut_Calloc(*ppstrEndeKey_m, TB_MAX, CHAR);	 /* Speicher ausgesch”pft   */
...
for(pF=pTB->pFeld; pF; pF=pF->pNext)
  ...
  Wi_TestPrintf(pWkbInfo_g, "\npTB(%d): ฏ%sฎ, ฏ%sฎ.", wL, pTB->acText, pstr);}
