...
D_ch_verify (flag)
    int flag;
{
bdos (CH_VERIFY, 0, (BYTE) !flag);      /* 1=gesetzt, 0=nicht */
}
...
inregs.h.al = (BYTE) (cmd ? 1 : 0);          /* 1 = SET_STATE */
inregs.h.dl = (BYTE) (state ? 1 : 0);           /* 1 = Setzen */
...
if (c_code < 0xff)
    inregs.h.al = (BYTE) c_code;
else
    {
    inregs.h.al = 0xff;
    inregs.x.bx = (WORD) c_code;
    }
inregs.x.dx = (WORD) c;
