...
scanf ("%64s", path);
printf ("\n%s wird geöffnet und die ersten 512 Bytes "
        "gelesen\n", path);
...
if (error)
    ...
    printf ("File Pointer jetzt an "
            "Position %ld; = Dateilänge\n", pos);
    D_h_close (fhandle);
    path [0] = '\0';   /* Temporäre Datei im Hauptverzeichnis */
    fhandle = D_creat_tmp (path, 0x20, &error);
    if (error)
         printf ("Beim Öffnen einer TMP-Datei"
         ...
         if (!D_h_close (fhandle))
              ...
              printf ("Fehler beim Schließen der TMP-Datei\n");
         }
    }
}
