 ...
 *                    Verfügung, die mit dem DOS-Directory zu    *
 *                    tun haben.                                 *
 *   Memory Model:    SMALL                                      *
...
inregs.x.dx = (SWORD) path;    /* Pointer auf Verzeichnisnamen */
...
/* ---------------------------------------------------------- *
 *  F 41h - Löscht Verzeichniseintrag und damit Datei         *
 * ---------------------------------------------------------- */
...
inregs.h.ah = DELETE;
inregs.x.dx = (SWORD) path;	    /* Pointer auf Verzeichnis */
...
/* ---------------------------------------------------------- *
 *  F 47h - Gibt aktuelles Verzeichnis eines Laufwerks zurück *
 * ---------------------------------------------------------- */
...
inregs.x.si = (SWORD) path;	/* Freier Speicher (64 Bytes!) */
inregs.h.dl = (SBYTE) drive;  /* Laufwerksnr. 0=aktuelles, 1=A */
...
static SWORD D_dta [4] = { 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF };
...
movedata (D_dta [1], D_dta [0], segregs.ds, (SWORD) f,
          sizeof (struct FIND));
D_dta [2] = segregs.ds;      /* Für die Funktion D_find_next() */
D_dta [3] = (SWORD) f;
return (0);
}
...
/* ---------------------------------------------------------- *
 *  F 4Fh - Sucht nächsten passenden Eintrag im Verzeichnis   *
 * ---------------------------------------------------------- */
...
if (D_dta [2] == 0xffff)  /* FIND_FIRST noch nicht aufgerufen */
    return (1);                  /* Ungültige Funktionsnummer */
...
inregs.x.dx = (SWORD) path1;			 /* Alter Name */
inregs.x.di = (SWORD) path2;			 /* Neuer Name */
