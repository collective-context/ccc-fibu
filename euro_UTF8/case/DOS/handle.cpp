...
inregs.h.ah = H_CREAT;
inregs.x.dx = (SWORD) path;			   /* Pfadname */
inregs.x.cx = (SWORD) fattr;		      /* Dateiattribut */
...
inregs.h.al = (SBYTE) beg;	    /* Anfang, Ende oder Mitte */
inregs.x.bx = (SWORD) handle;		      /* Handle-Nummer */
...
if (inregs.x.cflag)                                 /* Fehler */
    ...
    *error = inregs.x.ax & 0x00ff;
    ...
    {
    *error = 0;
    wptr = (SWORD *) &xx;	  /* Erstellen des long-Wertes */
...
inregs.x.bx = (SWORD) handle1;
inregs.x.cx = (SWORD) handle2;
