...
IMPORT CHAR	strApp_g[];
IMPORT PSTR     pstrDataBuffer_g;

STATIC WORD File_Definition (WORD, PPSTR, PFSPEC[], PSTR, WORD);
STATIC WORD i_AnalyseKeys (BFILEINFO, PFSPEC, WORD, PSTR, WORD);
STATIC WORD i_ExchangeKeyInfo (BFILEINFO, WORD, WORD);
...
/*╔════════════════════════════════════════════════════════════════════════╗
  ...
  ╚════════════════════════════════════════════════════════════════════════╝*/
GLOBAL
WORD Db_BOpen(WORD   wFile,	       PPSTR ppstrBlock,
	      PPSTR  ppstrOwner,       WORD  wLock,
	      PPSTR  ppstrFileName,    PWORD pwAccessMode,
	      PFSPEC apfsFileBuffer[], PWORD pwKeyLen,
	      PWORD  pwKeyOffset,      PPSTR ppstrKey,
	      PWORD  pwMaxKey,	       BFILEINFO aFiles[],
	      PSTR   pF, WORD wL)
{
REGISTER i;
WORD wOwnerLength, wCreate=0;			       /* Länge Besitzername  */
WORD wFileBufferLength;                                /* Länge Filenamen     */
WORD wStatus=0, wKeyNumber=0, wNull=0;
BOOL boSypara;
...
for(i=0,wStatus=12; (wStatus==11 || wStatus==12) &&
  ...
  {
  WORD wHandle=0;
  PSTR pstr, pstrName=strrchr(strFileName, '\\');
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ...
  ╚═════════════════════════════════════════════════════════════════════════╝*/
STATIC
WORD i_AnalyseKeys(BFILEINFO FileInfo, PFSPEC apfsFile,
		   WORD wFile, PSTR pF, WORD wL)
{
WORD awFound[24], wTheorie, wPraxis, i, j, wFound;
BOOL boOK;
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ...
  ╚═════════════════════════════════════════════════════════════════════════╝*/
STATIC
WORD i_ExchangeKeyInfo(BFILEINFO File, WORD wDest, WORD wSrc)
{
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ...
  ╚═════════════════════════════════════════════════════════════════════════╝*/
STATIC
WORD File_Definition(WORD wFH, PPSTR ppstrFileName,
       PFSPEC apfsFileBuffer[], PSTR pF, WORD wL)
{
REGISTER i;
WORD wDup, wMod, wDes, wAlt, wNul, wSeg,
...
PSTR   pstrPos;
WORD   wBufferLength, wKeyNumber=0, wNull=0;
WORD   wRetCode;
PSTR   pstrAltName;
CHAR   strKeyName[TB_MAX];
PSTR   pstrKeyName=strKeyName;

/*pWkbInfo = Wi_Einrichten (3,11,72,11);
Wi_SchattenEin (pWkbInfo);*/

stradd(strFileName, strExe_g, strApp_g, "MASK.", strExt_g, NULL);
/*stradd(strFileName, strExe_g, "EURODATA.BTR", NULL);*/
...
/*.ta B_Open(0)
╔══════════════════════════════════════════════════════════════════════════════╗
║  B_Open()   ... öffnet eine Btrieve-Datei unter einer Dateinummer.           ║
╚══════════════════════════════════════════════════════════════════════════════╝

Überblick:
────────────────────────────────────────────────────────────────────────────────
#include <eur_btr.h>
WORD B_Open(wFile, apstrBlock, apstrRecord, awRecordLength,
             apstrFileName, awAccessMode);

Parameter:
────────────────────────────────────────────────────────────────────────────────
WORD   wFile	   Logische Dateinummer.
PSTR   apstrBlock[]   Zeiger-Array auf Positionsblöcke.
PSTR   apstrRecord[]      Array von Zeigern auf String je Datensatz.
WORD   awRecordLength[]   Array, daß die Datenstatzlänge je Datei enthält.
PSTR   apstrFileName[]    Array aller Datei-Namen.
WORD   awAccessMode[]     Array, daß den Zugriffsmode für jede Datei speichert.
...
Die Dateien müssen zuvor irgendwann mit der »B_Create()« Funktion erzeugt und
damit vorhanden sein. Wird versucht eine Datei zu eröffnen, welche nicht
vorhanden ist, so wird der Fehlerstatus 12 zurückgegeben.
...
Btrieve erlaubt bis zu 255 gleichzeitig geöffnete Dateien. Wieviele Dateien
wirklich gleichzeitig geöffnet werden können hängt also von der Konfiguration
des DOS und vom verwendeten Compiler ab.
...
STATIC WORD File_Definition   (WORD, PSTR[], PFSPEC[], PSTR, WORD);
STATIC WORD i_AnalyseKeys     (BFILEINFO, PFSPEC, WORD, PSTR, WORD);
STATIC WORD i_ExchangeKeyInfo (BFILEINFO, WORD, WORD);
...
/*╔════════════════════════════════════════════════════════════════════════╗
  ...
  ╚════════════════════════════════════════════════════════════════════════╝*/
GLOBAL
WORD Db_BOpen (WORD wFile, PSTR apstrBlock[],
	      PSTR apstrOwner[], WORD wLock,
              PSTR apstrFileName[], WORD awAccessMode[],
	      PFSPEC apfsFileBuffer[], WORD awKeyLen[],
	      WORD awKeyOffset[], PSTR apstrKey[],
	      PWORD pwMaxKey, BFILEINFO aFiles[], PSTR pF, WORD wL)
{
REGISTER i;
WORD wOwnerLength;                                     /* Länge Besitzername  */
WORD wFileBufferLength;                                /* Länge Filenamen     */
WORD wStatus = 0;
BOOL boSypara;
CHAR strExtFileName[TB_MAX];
CHAR strOwner[TB_MAX];
CHAR strError[TB_MAX];

if(apstrBlock[wFile]==NULL)
  Ut_Calloc(apstrBlock[wFile], 128, CHAR);

if(apfsFileBuffer[wFile]==NULL)
  Ut_Calloc(apfsFileBuffer[wFile], 1, FSPEC);

sprintf(strOwner, "%sSYPARA.BTR", strDat_g);
if(!stricmp(strOwner, apstrFileName[wFile]))
  {
  boSypara=YES;
  strcpy(strOwner, "TFOSorue");
  apstrOwner[wFile]=strOwner;
  }
else
  boSypara=NO;

if (apstrOwner[wFile])				/* Falls Besitzername  */
   wOwnerLength = strlen(apstrOwner[wFile]) +1; /* da, Länge speichern */
else                                                   /* sonst ist die Be-   */
   wOwnerLength = 0;                                   /* sitzernamenlänge 0  */

wStatus=BTRV(B_OPEN+wLock, apstrBlock[wFile],	      /*		     */
		   apstrOwner[wFile],		/*		       */
                   &wOwnerLength,                      /*                     */
		   apstrFileName[wFile],	/*		       */
		   awAccessMode[wFile]);	/*		       */
...
if (wStatus == 12 && !boSypara)                        /* Wenn Datei nicht da */
   ...
   File_Definition(wFile, apstrFileName,
     apfsFileBuffer, pF, wL);
   ...
   wStatus=BTRV(B_CREATE, apstrBlock[wFile],/*			   */
               pstrDataBuffer_g,                       /*                     */
               &wFileBufferLength,                     /*                     */
	       apstrFileName[wFile],  NULL);	/*		       */

   sprintf(strError,"B_Create - Datei: "	  /*			 */
     "%d _ %s", wFile,		   /*			  */
     apstrFileName[wFile]);			/*		       */
   Dl_ErrorHandler(wStatus,strError,pF,wL,0);
...
if (boSypara)
  apstrOwner[wFile] = NULL;

sprintf(strError, "B_Open(%d) - Datei: "	      /*		     */
  "%d _ %s", wLock, wFile,
  apstrFileName[wFile]);			/*		       */
...
do
 {
  wFileBufferLength = sizeof(FSPEC);                   /*                     */
  wStatus = BTRV (B_STAT, apstrBlock[wFile],/*			   */
	  (PSTR) apfsFileBuffer[wFile], 	/*		       */
          &wFileBufferLength,                          /*                     */
          strExtFileName, NULL);                       /*                     */

  sprintf(strError, "B_Stat - Datei: "		  /*			 */
	  "%d _ %s", wFile,
          strExtFileName);                             /*                     */
  Dl_ErrorHandler(wStatus,strError,pF,wL,0);
  ...
  if (!wFile)
    ...
    for(i=0; i<apfsFileBuffer[0]->wNdxCnt; i++)       /* Für jeden Schlüssel */
      {
      if(apstrKey[i]==NULL)			      /* in der Stammdatei   */
	Ut_Calloc(apstrKey[i], TB_MAX, CHAR);	      /* Platz allokieren    */

      awKeyOffset[i] =				      /* Offset des	     */
        apfsFileBuffer[0]->ksKeyBuf[i].wKeyPosition-1;/* Schlüssels          */

      awKeyLen[i] =				/* Länge des	       */
        apfsFileBuffer[0]->ksKeyBuf[i].wKeyLength;    /* Schlüssels          */
      }
...
/* spricht. Außerdem wird auf eine ev. vorhandene Alternate Collating Sequ.  */
/* Rücksicht genommen. Die hier verwendete Technik mit einem return-Wert     */
/* und die in der aufrufenden Funktin vorkommende while-Schleife haben daher */
/* nur im Hinblick auf zukünftige Erweiterungen einen Sinn.                  */
...
/*╔═════════════════════════════════════════════════════════════════════════╗
  ...
  ╚═════════════════════════════════════════════════════════════════════════╝*/
STATIC
WORD File_Definition(WORD wFH, PSTR apstrFileName[],
       PFSPEC apfsFileBuffer[], PSTR pF, WORD wL)
{
REGISTER i;
WORD wDup, wMod, wDes, wAlt, wNul, wSeg,
...
strcpy (pstrKeyName, apstrFileName[wFH]);            /* Im Datei-Namen mit   */
while(*pstrKeyName!='.' && *pstrKeyName!='\0')       /* Pfad (..\ST1100.BTR) */
  pstrKeyName++;                                     /* den Punkt suchen und */
...
wBufferLength = 0;
wRetCode=BTRV(B_OPEN, strBlock, strBuffer, &wBufferLength, strFileName, O_NORMAL);
   sprintf (strError, "B_OPEN, - DATEI: %s", strFileName);
   Dl_ErrorHandler(wRetCode,strError,pF,wL,0);

wBufferLength = 50;
wRetCode = BTRV (B_GET_EQ, strBlock, strBuffer, &wBufferLength, pstrKeyName, 0);
if (wRetCode == 22 || wRetCode == 0)
   {
    sscanf (strBuffer, "%5d", &wBufferLength);
    Ut_Calloc(pstrBuffer, wBufferLength+1, CHAR);
    wRetCode = BTRV (B_GET_EQ, strBlock, pstrBuffer, &wBufferLength, pstrKeyName, 0);
   }
        sprintf (strError, "B_GET_EQ, - SCHLÜSSEL : >%s<.", pstrKeyName);
	Dl_ErrorHandler(wRetCode,strError,pF,wL,0);

wRetCode = BTRV (B_CLOSE, strBlock, pstrBuffer, &wBufferLength, strFileName, 0);
        sprintf (strError, "B_CLOSE, - DATEI: %s", strFileName);
	Dl_ErrorHandler(wRetCode,strError,pF,wL,0);
...
wKey=apfsFileBuffer[wFH]->wNdxCnt;
for(i=0, pstrPos = pstrBuffer+210; i<wKey; i++, pstrPos+=79)
  {
  WORD wNullValue;
  ...
  if (pstrPos-pstrBuffer >= wBufferLength)
    {
    sprintf (strError, "B_CREATE, - DATEI: %s", strFileName);
    Dl_ErrorHandler(120,strError,pF,wL,0);
    }
