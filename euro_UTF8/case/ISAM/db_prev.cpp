...
/*.ta Db_Prev
╔══════════════════════════════════════════════════════════════════════════════╗
║  Db_Prev()   ... geht zum vorherigen Datensatz in einer DBMS-Datei.          ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
KHK: Achtung: Wurde der zuletzt gelesene Datensatz nicht mit einer Schreibsperre
  ...
  ------ Read_Back ------
   Diese Funktion liefert den in der Sortierfolge vorigen  Schlüssel mit seinem
   zugehörigen Datensatz. Befindet sich der Zeiger für sequentielles Lesen
   bereits am Dateianfang, so wird ein entsptechender Rückgabestatus geliefert.
   ...
   anschließenden Read_Prev () wird also der in der Sortierfolge nächst minder-
   wertige Schlüssel gelesen, der in diesem Falle ungleich dem direkt gelesenen
   ist.
   Versorgung:
      Eingang:
      int file_handler;
      ...
      Ausgang:
      char *key;
         Beinhaltet nach erfolgreichem Lesen den vorhergehenden Schlüssel aus
         der Sortierfolge. Wenn kein weiterer Schlüssel gefunden werden konnte,
      ...
      int end_flag;
         ...
         Lesen bereits am log. Dateianfang steht und somit kein Schlüssel und
  ...
  ------ Read_Le ------
   ...
   Schlüssel gefunden werden, so wird der nächst kleiner  Schlüsselwert (unab-
   hängig von der Sortierfolge) genommen. Existiert kein nächst kleinerer
   ...
   Versorgung:
      ...
      int end_flag;
         ...
         nächst kleinerer Schlüssel existiert und somit auch kein Datensatz
