...
itoa(atoi(pstrNew+i)+wZahl,acTemp,10);		    /* String um wZahl änd.*/
if(strlen(acTemp)>strlen(&pstrNew[i]) )              /* Wenn Zahl länger,    */
  strcpy(pstrNew+i, acTemp);                         /* dann Zahl anhängen,  */
else                                                 /*                      */
  {                                                  /* sonst:               */
  memset(pstrNew+i,'0',strlen(pstrNew+i));           /* mit NULL auffüllen.  */
...
/* Hier könnten auch monatsübergreifende Erhöhungen realisiert werden !!! */
...
if(*pstr=='+'||*pstr=='t'||*pstr=='m'||*pstr=='j')   /* Bei + : 	     */
  ...
  if(wZahl<1) wZahl=1;				   /* Mindest 1 dazuzählen */
  }                                                  /*                      */
...
if(pstrOld==NULL)
  ...
  Ut_SchreibArray(apstrMessage,
    ...
    "───────────────────────────────────────────────────────────── ",
    "                                                              ",
    "Tip: Manche Programme verwenden außerdem eigene Nummernkreise.",
    "     Durch die Eingabe von [&][┘] oder [┘] wird Ihnen dann  ",
...
itoa(atoi(pstr+i)+wZahl,acTemp,10);		    // String um wZahl änd.
if(strlen(acTemp)>strlen(&pstr[i]) )                 // Wenn Zahl länger,
  strcpy(pstr+i, acTemp);                            // dann Zahl anhängen,
else                                                 //
  {                                                  // sonst:
  memset(pstr+i,'0',strlen(pstr+i));                 // mit NULL auffüllen.
...
//║────────────────────────────────────────────────────────────────────────║
//║  in m_botore.c							   ║
//║  ... könnte bei monatsübergreifenden Erhöhungen verwendet werden !!!   ║
//╚════════════════════════════════════════════════════════════════════════╝
STATIC
